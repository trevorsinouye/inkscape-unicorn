...
class GCodeContext:
    def __init__(self, xy_feedrate, z_feedrate, start_delay, stop_delay, pen_up_angle, pen_down_angle, z_height, finished_height, x_home, y_home, register_pen, num_pages, continuous, file):
      ...
      self.postscript = [
        "",
				"(end of print job)",
				"M05 S%0.2F # (pen up)" % self.pen_up_angle,
      ...
      self.registration = [
        "M03 S%d (pen down)" % (self.pen_down_angle),
        "G4 P%d (wait %dms)" % (self.start_delay, self.start_delay),
        "M05" # S%d (pen up)" % (self.pen_up_angle),
        "G4 P%d (wait %dms)" % (self.stop_delay, self.stop_delay),
    ...
    def start(self):
      self.codes.append("M03 S%0.2F (pen down)" % self.pen_down_angle)
      self.codes.append("G4 P%d (wait %dms)" % (self.start_delay, self.start_delay))
      self.drawing = True

    def stop(self):
      self.codes.append("M05")# S%0.2F (pen up)" % self.pen_up_angle)
    ...
    def draw_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing == False:
            self.codes.append("M03 S%0.2F (pen down)" % self.pen_up_angle)
            self.codes.append("G4 P%d (wait %dms)" % (self.start_delay, self.start_delay))
            self.drawing = True
